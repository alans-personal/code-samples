...
import traceback
import boto3
import util.awscost_helper_util as helper_util
...
def lambda_deploy_handler(event, context):
    """
    Deploy the awscost lambda function to either dev or prod lambdas.
    :param event:
    :param context:
    :return:
    """
    print('Starting: AWS Cost pipeline deployment. v1.0 - 20181116-1621')
    ...
    try:
        ...
        if deploy_stage == 'dev':
            ...
            helper_util.print_delta_time(
                start_time, 'Finished: awscost-dev')
            ...
            helper_util.print_delta_time(
                start_time, 'Finished: awscost-controller')

            update_lambda_function_code(
                'awscost-pipeline-deploy-dev',
                s3bucket, s3key
            )
            helper_util.print_delta_time(
                start_time, 'Finished: awscost-pipeline-deploy-dev')

            print('Deploy to cost-panda')
            update_lambda_function_code(
                'awscost-panda',
                s3bucket, s3key
            )
            helper_util.print_delta_time(
                start_time, 'Finished: awscost-panda')

        elif deploy_stage == 'prod':
            error_msg = "PROD deploy stage is deprecated. Expected 'dev'|'prod'. Was {}.".format(deploy_stage)
            raise CodePipelineStageError(error_msg)
            # DEPRECATED.  dev seems to be prod.
            # update_lambda_function_code(
            #     'awscost-prod',
            #     s3bucket, s3key, True
            # )
            # helper_util.print_delta_time(
            #     start_time, 'Finished: awscost-prod')

        else:
            error_msg = "Unknown deploy stage. Expected 'dev'|'prod'. Was {}.".format(deploy_stage)
            raise CodePipelineStageError(error_msg)
        ...
        slack_msg_text = "Deployed AwsCost {}".format(deploy_stage)
        slack_msg_text += '\nPipeline version: {}'.format(helper_util.get_aws_cost_version())

        if extra_build_info is not None:
            slack_msg_text += '\nCost Version: %s' % extra_build_info
...
def upload_and_unzip_source_code(
    ...
    helper_util.unzip_file(dest_zip_file_path, unzip_dir)
...
def get_build_info_from_zip(unzip_dir):
    ...
    try:
        txt_files = helper_util.get_files_in_dir_by_type(
            unzip_dir, 'txt'
        )
        ...
        if len(txt_files) > 0:
            ...
            build_info_map =\
                helper_util.read_key_value_file_to_dictionary(
