...
def generate_sub_command_prop_methods(new_cmd_lower_case, sub_command_list):
    ...
    for curr_sub_cmd in sub_command_list:
        ...
        ret_val += "            'help_text': '*`{}`* _description here_',\n".format(curr_sub_cmd)
        ret_val += "            'help_examples': [\n"
        ret_val += "                '/run {} {} -e sr-dev -r us-east-1 -s protoservice',\n".format(
            new_cmd_lower_case, curr_sub_cmd)
        ret_val += "                '/run {} {} -e sr-dev -r us-west-2 -s protoservice -c changeme'\n".format(
            new_cmd_lower_case, curr_sub_cmd)
        ret_val += "            ],\n"
...
def main():
    ...
    print("Which group is this command for?")
    print("Here are the valid options.")

    # Keep this in sync with ../slack_bud/util/group.py
    valid_groups_dict = {
    ...
    group_keys = valid_groups_dict.keys()
    group_keys.sort()
    for curr_group_key in group_keys:
        curr_desc = valid_groups_dict.get(curr_group_key)
        print('   {:<8}: {:<24}'.format(curr_group_key, curr_desc))
    cmd_group = raw_input("\n Type a group from above or 'all':")
    cmd_group = cmd_group.strip()
    cmd_group = cmd_group.lower()

    valid_group = False
    if cmd_group in group_keys:
        valid_group = True
    elif cmd_group == 'all':
        valid_group = True

    if not valid_group:
        print("\nNot a valid group: {}".format(cmd_group))
        print('Please select a valid group: [{}]'.format(valid_group))
        return
    ...
    template_string = template_string.replace('{#cmd_group#}', cmd_group)
    ...
    if success:
        ...
        if success:
            ...
            print("Check /rundev in a few minutes for new command.")
