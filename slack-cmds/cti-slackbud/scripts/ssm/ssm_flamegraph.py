...
def fg_script(docker_id, run_time, comment):
    ...
    sys_call(fd, 'perf script --header > perf-source-{}.perf'.format(pid))
    sys_call(fd, 'cat perf-source-{}.perf | /usr/local/flamegraph/stackcollapse-perf.pl --kernel | /usr/local/flamegraph/flamegraph.pl --color=java --hash > perf-{}.svg'.format(pid, pid))
    ...
    sys_call(fd, 'aws s3 cp /home/ec2-user/{} s3://slackbud-flamegraph/svg/{}'
                 ' --acl public-read'.format(svg_file, dest_svg_file_name))

    # Copy perf-source.$PID.perf into s3 bucket.
    perf_source_file = 'perf-source-{}.perf'.format(pid)
    dest_perf_source_file_name = 'perf-source-{}-{}.perf'.format(pid, name_post_fix)
    find_perf_source_file = sys_call(fd, 'find / -type f -name {}'.format(perf_source_file))
    output(fd, "find_perf_source_file=_{}_".format(find_perf_source_file))
    sys_call(fd, 'aws s3 cp /home/ec2-user/{} s3://slackbud-flamegraph/svg/{}'
                 ' --acl public-read'.format(perf_source_file, dest_perf_source_file_name))
