...
def zip_slack_bud_dir():
    ...
    if os.environ.get('GITPATH'):
        ...
        # root_dir = os.environ.get('CI_PROJECT_DIR')
...
def upload_zip_to_s3(role_name):
    """
    ...
    print('---------')
    cmd = 'aws s3 cp ../slack-bud-bare-repo-files.zip s3://cti-pipeline-source/slack-bud-bare-repo-files.zip --profile {}'.format(role_name)
    print(cmd)
    os.system(cmd)
    print('^^^^^^^^^')
...
def download_githook_info_file(role_name):
    """
    Download the githool_info.txt file to a temp location to update.
    :return:
    """
    cmd = 'aws s3 cp s3://cti-pipeline-source/githook_info.txt temp_githook_info.txt --profile {}'.format(role_name)
    print(cmd)
    os.system(cmd)


def upload_githook_info_file(role_name):
    """
    ...
    if os.path.exists('temp_githook_info.txt'):
        ...
        cmd = 'aws s3 cp temp_githook_info.txt s3://cti-pipeline-source/githook_info.txt --profile {}'.format(role_name)
        print(cmd)
        os.system(cmd)
...
def ask_role():
    result = input("Use CTI-Dev role [Y|N]?")
    result = result.lower()
...
def main():
    ...
    try:
        ...
        download_githook_info_file(role_name)
        commit = get_latest_git_info()
        update_githook_info_file(commit)
        upload_zip_to_s3(role_name)
        # upload_build_info_file()
        upload_githook_info_file(role_name)
        delete_zip_file()
