...
def lambda_send_email_handler(event, context):
    ...
    recipients = ['asnyder@roku.com', 'asnyder@roku.com']
    # ToDo: get recipient from githook info. Send to last code committer.

    msg = MIMEMultipart()
    msg['Subject'] = subject_line
    msg['From'] = 'REDACTED-Service@roku.com'
...
def update_lambda_function_code(function_name, s3_bucket, s3_key,
    ...
    try:
        ...
        error_log = 'Failed to update function {}. Reason: {}'.format(function_name, ex)
        if fail_stage_on_error:
            raise CodePipelineStageError(error_log)
        else:
            print(error_log)
...
def post_message_to_slack_channel(title, text, color='#36a64f'):
    ...
    url = "https://hooks.slack.com/services/T025H70HY/B8SAM0LRY/bC__REDACTED__u3hr"  # TODO: Replace with actual Slack webhook URL
    res = requests.post(
        url=url,
...
def lambda_smoke_and_unit_test_handler(event, context):
    ...
    try:
        ...
        # Run the smoke test with a `/rundev help`
        ctidev_help_event = create_cti_slack_help_event_for_lambda()
        # do an invoke here.
        lambda_client = boto3.client('lambda')

        bud_helper_util.print_delta_time(
            start_time, 'Start: invoke ctidev help')

        response = lambda_client.invoke(
            FunctionName='serverless-slackbud-dev-slackBud',  # This is ctidev
            InvocationType='RequestResponse',
            LogType='Tail',  # Need to see if this give CloudWatch result.
            Payload=json.dumps(ctidev_help_event)
        )

        bud_helper_util.print_delta_time(
            start_time, 'Finished: invoke ctidev help')
        ...
        string_in_log = "import_class_package=cmds.cmds_version.CmdVersion"
...
def create_cti_slack_help_event_for_lambda():
    ...
    lambda_event = {
        'body':
            ...
            'response_url=https%3A%2F%2Fhooks.slack.com%2Fcommands%2FT025H70HY%2F335764193537%2FOCH__REDACTED__ANiI&'  # TODO: Replace with actual Slack webhook URL
        ...
        'headers': {
            ...
            'Via': '1.1 738__REDACTED__.cloudfront.net (CloudFront)',
...
# Need to make CTI version.  Is this deprecated?
def create_test_event_for_lambda(entire_cli_str):
    ...
    str_text = {
        ...
        'response_url': 'http://int-res-url.infra.sr.asnyder.com'
    }

    check_text = urllib.urlencode(str_text)

    request_body = 'token=jZC__REDACTED__RdGK5&' \
                   'team_id=T025H70HY&' \
                   'team_domain=roku&' \
                   ...
                   '{}&' \
                   'trigger_id=' \
                   '123456789012.123456789012.be67e6__REDACTED__6ca7c2035'  # TODO: Replace with actual Slack webhook URL
    ...
    event = {
        ...
        "requestContext": {
            ...
            "accountId": "141602222194",
            "stage": "qa"
        },
        "headers": {
            "Via": "1.1 c22__REDACTED__.cloudfront.net (CloudFront)",
            "Accept-Encoding": "gzip,deflate",
            "CloudFront-Is-SmartTV-Viewer": "false",
            "CloudFront-Forwarded-Proto": "https",
