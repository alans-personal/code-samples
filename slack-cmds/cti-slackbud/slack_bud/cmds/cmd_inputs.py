...
class CmdInputs:
        ...
        try:
            if params:
                ...
                if not self._is_confirmation_cmd:
                    ...
                    self._slack_user_id = get_slack_user_id_from_params(params)
                    ...
                    if self._raw_cmd_line:
                        print('--------------\nRAW_CMD_LINE\n--------------')
                        print('{}'.format(self._raw_cmd_line))
                        self._arg_list = [p for p in re.split("( |\\\".*?\\\"|'.*?')", self._raw_cmd_line) if p.strip()]
                        cmd = str(self.get_by_index(0))
                        self._cmd = cmd.lower()
                else:
                    print('(debug) CONFIRMATION params: {}'.format(params))
                    # If this a BlockKit Response?
                    if has_callback_id(params):
                        self._callback_id = get_callback_id(params)
                        self._callback_value = get_fallback_value(params)
                    else:
                        print('LIKELY BlockKit response.')
                        # ToDo: figure out how to get an id needed for a response.
                        ### Start Hack-a-thon temp code
                        # push the trigger_id into callback_id
                        self._callback_id = hackathon_get_trigger_id(params)
                        self._callback_value = 'n/a'
                        ### End Hask-a-thon temp code
            ...
            squash_token_print("Failed to parse params = ", params)
    ...
    def serialize(self):
        ...
        ret_val = {
            ...
            'slack_user_group': none_to_empty_str(self._slack_user_group),
            'cmd': self._cmd,
    ...
    def deserialize(self, cmd_inputs_dict):
        ...
        self._slack_user_group = empty_str_to_none(cmd_inputs_dict['slack_user_group'])
    ...
    def log_state(self, prefix):
        ...
        print('{} CmdInput: slack_user_group = {}'.format(prefix, self._slack_user_group))
...
def hackathon_get_trigger_id(params):
    try:
        data = json.loads(params['payload'][0])
        trigger_id = 'no_trigger_id'
        trigger_id = data.get('trigger_id')
        return trigger_id.strip()
    except Exception as ex:
        print ('ERROR, hackathon_get_trigger_id, reason: {}'.format(ex))


def has_callback_id(params):
    try:
        get_callback_id(params)
        return True
    except ValueError as ve:
        print('No callback id. reason: {}'.format(ve))
        return False
...
def get_slack_user_id_from_params(params):
    """
    Private utility method to just get the use_id from the call.
    :param params: params
