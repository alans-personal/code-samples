...
from util.TagCounter import TagCounter
...
class CmdUntagged(CmdInterface):
    ...
    def get_cmd_properties(self):
        ...
        props = {
            'group': 'all',
            'sub_commands': ['report', 'types', 'accounts'],
            ...
            'props_accounts': self.get_accounts_properties()
        }
    ...
    def get_report_properties(self):
        ...
        props = {
            'run-type': 'longtask',
            'help_text': '*`report`* _Create report of all untagged AWS resources of the type(s) specified_',
            'help_examples': [
                '/run untagged report -e cti -r us-east-1 -t s3 -o excel',
                '/run untagged report -e sr-dev -r us-east-1 -t ec2,s3,dynamo -o text',
                '/run untagged report -e apps-qa -r us-east-1 -t all -o excel'
            ],
    ...
    def invoke_report(self, cmd_inputs):
        ...
        try:
            ...
            aws_type_list = []
            if arg_type == 'all':
                aws_type_list = ['cloudfront', 'dynamo', 'ec2', 'elasticache', 'elb', 's3']
            ...
            # Add the Spend_Category count page.
            spend_category_data_frame = create_spend_category_count_frame()
            data_frame_dict.update({'spend_categories': spend_category_data_frame})
    ...
    def get_accounts_properties(self):
        """
        ...
        props = {
            'run-type': 'shorttask',
            'help_text': '*`accounts`* _Shows all the valid accounts to use for *-e* param in report command_',
            'help_examples': [
                '/run untagged accounts'
            ],
            'switch-templates': [],
        }
        return props

    def invoke_accounts(self, cmd_inputs):
        """
        ...
        """
        try:
            print("invoke_accounts")
            response_url = cmd_inputs.get_response_url()

            # Start Types code section #### output to "text" & "title".
            text = "Account names (-e) param: \n"

            aws_account_list = cti_helper_util.list_aws_account_names()
            aws_account_list.sort()

            for curr_key in aws_account_list:
                text += '    *{}*\n'.format(curr_key)
            # End Types code section. ####

            # Standard response below. Change title and text for output.
            title = "Roku's AWS Accounts available"
            return self.slack_ui_standard_response(title, text)
        except ShowSlackError:
            raise
        except Exception as ex:
...
def create_excel_file_in_s3_bucket(data_frame_dict, arg_env, arg_region):
    ...
    date_str = datetime.datetime.today().strftime('%b%d-%H%M')
    ...
    # move spend_categories toward front.
    if keys.count("spend_categories") == 1:
        keys.remove("spend_categories")
        keys.insert(0, "spend_categories")

    # move summary page to front.
    if keys.count("summary") == 1:
        keys.remove("summary")
    ...
    s3.meta.client.upload_file(
        path, bucket_name,
        'month/TagReport-{}-{}-{}.xlsx'.format(aws_account, aws_region, date_str)
    )

    link_path = 'month/TagReport-{}-{}-{}.xlsx'.format(aws_account, aws_region, date_str)
...
def make_camel_case(string):
    """
    Take a string with dash and make it camel case without dashes.
    :param string: String in format.   abc-xyz-1
    :return: String with dashes removed and camel case: AbcXyx1
    """
    if type(string) is not str and type(string) is not unicode:
        input_type = type(string)
        print(""
              "_{}_ is not string. It is type: {}. Won't make camel case".format(string, input_type))
        return string
    ...
    for part in parts:
        case_str = part.title()
        # Apply some exceptions, for AWS account names that should be upper case.
        part_lower = part.lower()
        if part_lower == 'cti' or part_lower == 'dea' or part_lower == 'bar':
            case_str = part.upper()
        ret_val += case_str
...
def convert_data_frame_to_text(data_frame_dict):
    ...
    for curr_key in keys:
        ...
        for index, row in curr_data_frame.iterrows():
            ...
            row_cost = row.get('cost-estimate')
            if not row_name or not row_cost:
                print('WARN: Skipping row: {}'.format(row))
            else:
                line = '{}, {}'.format(row_name, row_cost)
                print(line)
                text += '{}\n'.format(line)
...
def init_ec2_data_frame():
    ...
    :return: panda DataFrame object
    """
    ec2_type_data_frame = pandas.DataFrame(columns=['name', 'instance_id', 'cost-estimate', 'spend_category'])
...
def create_spend_category_count_frame():
    """
    Create a dataframe to summarize Spend_Category tags.
    :return:
    """
    spend_category_data_frame = pandas.DataFrame(columns=['Spend_Category', 'count'])

    tc = TagCounter()

    sorted_keys = tc.get_sorted_keys()
    for key in sorted_keys:
        value = tc.get_value(key)
        spend_category_data_frame.loc[len(spend_category_data_frame)] = [key, value]

    return spend_category_data_frame
...
def check_for_untagged_autoscaling_groups(session, region, tagging_client, summary_data_frame):
    ...
    try:
        ...
        template = 'Failed during Autoscaling phase. type {0} occurred. Arguments:\n{1!r}'
...
def check_for_untagged_cloud_front_distributions(session, region, tagging_client, summary_data_frame):
    ...
    try:
        ...
        # Report back an error to the user, but ask to check logs.
        template = 'Failed during CloudFront phase. type {0} occurred. Arguments:\n{1!r}'
        print(template.format(type(ex).__name__, ex.args))
        traceback_str = traceback.format_exc()
        print('Error traceback \n{}'.format(traceback_str))
...
def check_for_untagged_dynamo_tables(session, region, tagging_client, summary_data_frame):
    ...
    try:
        ...
        # Report back an error to the user, but ask to check logs.
        template = 'Failed during Dynamo phase. type {0} occurred. Arguments:\n{1!r}'
        print(template.format(type(ex).__name__, ex.args))
        traceback_str = traceback.format_exc()
        print('Error traceback \n{}'.format(traceback_str))
    ...
    try:
        ...
        for reservation in response["Reservations"]:
            for instance in reservation["Instances"]:
                ...
                spend_category_set.add(spend_category_tag)
                tag_counter.increment(spend_category_tag)

                # List any EC2 Instances without a Spend_Category tag or a value of unknown.
                if spend_category_tag == '' or spend_category_tag.lower() == 'unknown'.lower():
                    ec2_type_data_frame.loc[len(ec2_type_data_frame)] = [name_value, instance_id, instance_type, spend_category_tag]
        ...
        sc_tag_list = list(spend_category_set)
        sc_tag_list.sort()
        ret_val += 'Spend_Category values: {}'.format(sc_tag_list)
        print(ret_val)
...
def check_for_untagged_elasticache_replication_groups(session, region, tagging_client, summary_data_frame):
    ...
    :return: text in SlackUI format with report values.
    """
    print('def check_for_untagged_elasticache_replication_groups')
    ...
    try:
        ...
        # Report back an error to the user, but ask to check logs.
        template = 'Failed during Elasticache phase. type {0} occurred. Arguments:\n{1!r}'
        print(template.format(type(ex).__name__, ex.args))
        traceback_str = traceback.format_exc()
        print('Error traceback \n{}'.format(traceback_str))
