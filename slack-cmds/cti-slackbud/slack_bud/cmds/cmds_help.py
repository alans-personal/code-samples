...
# {#sub_command_prop_method_def#}
    ...
    def run_command(self):
        ...
        try:
            ...
            slack_user = cmd_inputs.get_slack_user_name()
            slack_user_group = cmd_inputs.get_user_group()
            print('Help looking up commands for user: {} in group: {}'.format(slack_user, slack_user_group))

            title = 'SlackBud Help'
            text = 'Available commands\n'
            # Walk though all of the Cmd* classes calling get_help_title().
            text += get_title_from_all_commands(slack_user_group)
            text += 'for help with a specific command use: \n'
            text += '_/run <command> help_\n'
            text += 'More info on confluence page: https://confluence.portal.asnyder.com:8443/display/SR/SR+Slack+BudE\n'
...
def get_title_from_all_commands(slack_user_group):
    """
    Get the title from all the groups, but return only the ones
    where a user has a match to the group, or the command group name is all
    :param slack_user_group:
    :return:
    """
    ret_val = ''
    try:
        ...
        for curr_module in all_cmd_modules_list:
            if curr_module.startswith(cmd_name_prefix):
                ...
                curr_cmd_group = klass(cmd_inputs).get_command_group()
                is_group_match = False
                user_group_list = [x.strip() for x in slack_user_group.split(',')]
                cmd_group_list = [x.strip() for x in curr_cmd_group.split(',')]
                print('TEMP: user_group_list=[{}] cmd_group_list=[{}]'.format(user_group_list, cmd_group_list))
                for cg in cmd_group_list:
                    if cg in user_group_list:
                        is_group_match = True
                    elif cg == 'all':
                        is_group_match = True
