...
SLACK_CMD_TABLE = DYNAMODB.Table('SlackBudCmds')
...
class CmdInterface:
    ...
    def get_command_group(self):
        """
        Get the group for the command.
        'all' is a special group meaning any group of course.
        :return: group like 'cti' | 'cti,sr' | 'all'
        """
        cmd_props = self.get_cmd_properties()
        group = cmd_props['group']+'\n'
        return group
    ...
    def authenticate_request(self, params):
        ...
        try:
            ...
            if self._cmd_inputs.is_confirmation_cmd():
                ...
                # TEMPORARY. "/run p4 status" is avail to everyone.
                if self._cmd_inputs.get_command() == 'p4':
                    return None
                ...
                if slack_user is None:
                    ...
                    text = "Request access. Ask an admin (@asnyder or @jscott) to add user to slack *`/run`* app"
                    raise ShowSlackError(text)
                    # return slack_ui_util.ephemeral_text_response(text)
                else:
                    ...
                    slack_user_group = slack_user['group']
                    self._cmd_inputs.set_user_group(slack_user_group)
                    print('Found Slack user: {}'.format(slack_user_name))
                    print('User role: {}'.format(slack_user_role))
                    print('User group(s): {}'.format(slack_user_group))
                    cmd_props = self.get_cmd_properties()
                    cmd_group = cmd_props['group']
                    # validate user in group
                    if not is_user_in_cmd_group(slack_user_group, cmd_group):
                        # failed validation.
                        raise ShowSlackError("User's group doesn't match command. See log for details.")
                    ...
                    if cmd_permission_level == 'admin' and slack_user_role == 'admin':
                        ...
                        print('ERROR: User not authorized to run command.')
    ...
    def slack_ui_build_kit_response(self, build_kit_dict):
        print('Build Kit Response: {}'.format(build_kit_dict))
        print("Type: ", type(build_kit_dict))
        if type(build_kit_dict) is not dict:
            raise ShowSlackError("UI response needs to be dictionary, got {} instead".format(type(build_kit_dict)))
        post = False
        ...
        build_kit_dict["response_type"] = response_type

        if where_am_i == 'shorttask':
            temp = 'This is a placeholder'
        elif where_am_i == 'longtask':
            post = True
            response_url = cmd_inputs.get_response_url()
            print("LONGTASK IDENTIFIED, RESPONSE URL: ", response_url)
        build_kit_json_str = json.dumps(build_kit_dict)
        if post:
            print("Start posting, headers = ", HEADER)
            response = requests.post(response_url, data=build_kit_json_str, headers=HEADER)
            print("Response text ", response.text)
            print("Finished posting ", print(response))
            return None

        return slack_ui_util.respond(None, build_kit_json_str)


    # Below is DEPRECATED
    # NOTE: This method is temporary for hack-a-thon will remove when done.
    def slack_ui_build_kit_test_response(self, id=None, attachment=True):
        print('Buid Kit Response. id={}'.format(id))

        cmd_inputs = self.get_cmd_input()
        where_am_i = cmd_inputs.get_where_am_i()
        print('slack_ui_build_kit_test_response at {}'.format(where_am_i))

        is_public_response = cmd_inputs.is_public_response()

        if where_am_i == 'shorttask':
            if '1' in id:
                return slack_ui_util.test_build_kit_response(is_public=is_public_response)
            if '2' in id:
                return slack_ui_util.test_build_kit_response2(is_public=is_public_response)
            if '3' in id:
                return slack_ui_util.test_build_kit_response3(is_public=is_public_response)
            # if attachment:
            #     return slack_ui_util.text_command_response(title, text, color=color,
            #                                                is_public=is_public_response, attachment=True)
            # else:
            #     return slack_ui_util.text_command_response(title, text, color=color,
            #                                                is_public=is_public_response, attachment=False)
        elif where_am_i == 'longtask':
            response_url = cmd_inputs.get_response_url()
            if '1' in id:
                return slack_ui_util.test_build_kit_response(is_public=is_public_response, response_url=response_url)
            if '2' in id:
                return slack_ui_util.test_build_kit_response2(is_public=is_public_response, response_url=response_url)
            if '3' in id:
                return slack_ui_util.test_build_kit_response3(is_public=is_public_response, response_url=response_url)
            # if attachment:
            #     return slack_ui_util.text_command_response(
            #         title, text, color=color, post=True, response_url=response_url,
            #         is_public=is_public_response, attachment=True)
            # else:
            #     return slack_ui_util.text_command_response(
            #         title, text, color=color, post=True, response_url=response_url,
        ...
        if '1' in id:
            return slack_ui_util.test_build_kit_response()
        if '2' in id:
            return slack_ui_util.test_build_kit_response2()
        if '3' in id:
            return slack_ui_util.test_build_kit_response3()

        print('Using default build kit')
        return slack_ui_util.test_build_kit_response()

    def slack_ui_standard_response(self, title, text, color='#a0ffaa', attachment=True):
        """
    ...
    def create_args_key_value_map(self):
        ...
        try:
            ...
            # -e sr-dev -r us-east-1 -s content --show
            next_index = 0
            kv_map = {}
    ...
    def apply_env_rules(self, optional=False):
        ...
        value = value.lower()
        # check valid values
        expect_list = cti_helper_util.list_aws_account_names()
        # Need to reconcile names with SR SlackBud.
    ...
    def default_run_command(self):
        ...
        # Add cmd history to database.
        where_am_i = cmd_inputs.get_where_am_i()
        if where_am_i == 'shorttask':
            self.__add_cmd_history_to_db()
    ...
    def __add_cmd_history_to_db(self):
        """
        Add a validated command line to the history table.
        If an error occurs log it, but don't stop the command from running.
        :return: None
        """
        try:
            raw_cmd_line = self._cmd_inputs.get_raw_inputs()
            user_id = self._cmd_inputs.get_slack_user_id()
            time_str = time.strftime('%y%m%d%H%M%S')

            # expire the row in 6 months.
            ttl = int(round(time.time()) + (3600*24*30*6))

            print('(DEBUG) calling put_item. userid={}, timestamp={}, ttl={}, cmdline={}'
                  .format(user_id, time_str, ttl, raw_cmd_line))

            response = SLACK_CMD_TABLE.put_item(
                Item={
                    'userid': user_id,
                    'timestamp': time_str,
            ...
            # Log the error, but don't stop the command.
            bud_helper_util.log_traceback_exception(ex)
...
def is_valid_token(params):
    ...
    print('DEBUG: getting token for: {}'.format(token_name))
...
def look_up_bud_user(params):
    ...
    print('TEMP: BUD_USERS_TABLE.get_item response: {}'.format(response))
    if 'Item' not in response:
        # Log this user_id
...
def is_user_in_cmd_group(slack_user_group, cmd_group):
    """
    Check that a user is allow to use a command based on which
    group(s) they are in.
    :param slack_user_group:
    :param cmd_group:
    :return: True if use group matches a command group. Otherwise False
    """
    # Everyone is allow to use 'all'.
    if cmd_group == 'all':
        return True

    # Get list of valid groups.
    valid_group_list = groups.get_valid_group_list()

    cmd_group_list = cmd_group.split(',')
    cmd_group_list = [x.lower() for x in cmd_group_list]
    verify_list_contains_elements(cmd_group_list, valid_group_list, 'Command')
    cmd_group_set = set(cmd_group_list)
    user_group_list = slack_user_group.split(',')
    user_group_list = [x.lower() for x in user_group_list]
    verify_list_contains_elements(user_group_list, valid_group_list, 'User')
    user_group_set = set(user_group_list)

    common_group_set = cmd_group_set.intersection(user_group_set)
    set_len = len(common_group_set)

    if set_len == 0:
        # Log an error.
        print('ERROR: User group not listed in command. User:{} Cmd:{}'.format(slack_user_group, cmd_group))
        return False

    return True


def verify_list_contains_elements(sub_list, check_list, type_of_list):
    """
    Verify that all elements of sub_list are in check_list.

    If any are invalid print details into log and return a ShowSlackError.

    :param sub_list: list with elements to check.
    :param check_list: list with all possible elements
    :param type_of_list: Type of list. Just used for logging
    :return: None, raise error if exception found.
    """
    not_in_list = []
    for curr_item in sub_list:
        if curr_item not in check_list:
            not_in_list.append(curr_item)

    if len(not_in_list) > 0:
        print('ERROR: Invalid groups in type: {}. Invalid items: {}'.format(type_of_list, not_in_list))
        raise ShowSlackError('Invalid groups. Check logs.')
