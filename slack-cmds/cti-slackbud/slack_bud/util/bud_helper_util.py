...
def get_props_table_name_for_stack_name(session, region, stack_name, service):
    ...
    try:
        ...
        for i, dic in enumerate(stack_description['Stacks'][0]['Outputs']):
            if 'PropertiesTable' in dic['OutputKey'] or 'PropertyTable' in dic['OutputKey']:
                index = i
                break
...
def get_slack_bud_environment(params):
    ...
    if cmd_list:
        ...
        if slack_cmd_word.endswith('buddev2'):
            ...
            return 'dev'
        elif slack_cmd_word.endswith('ctidev'):
            return 'dev'
        elif slack_cmd_word.endswith('rundev'):
            return 'dev'
        elif slack_cmd_word.endswith('cti'):
            return 'prod'
        elif slack_cmd_word.endswith('bud'):
            return 'prod'
        elif slack_cmd_word.endswith('run'):
            return 'prod'
        else:
            message = "Unexpected slack_cmd_word. Expect: " \
                      "('bud'|'buddev'|'buddev2'|'rundev'|'cti'|'run'). Was: {}".format(slack_cmd_word)
            raise BudHelperError(message)
    else:
...
def squash_token_print(message, event):
    ...
    if 'body' in event.keys():
        ...
        start_token = save_token[:3]
        end_token = save_token[-3:]
        token_len = len(save_token)
        replace_token = '{}..({})..{}'.format(start_token, token_len, end_token)
        print('DEBUG: replace_token-{}'.format(replace_token))

        event['token'] = replace_token
        print(message, ": {}".format(event))
